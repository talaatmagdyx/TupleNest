...
- **Expression indexes for filters a plain column index can't serve.** When the
  ...
  Only functions that are `IMMUTABLE` in **every** overload are offered, because
  `CREATE INDEX` rejects anything else and volatility depends on the argument
  type, which a plan never states. That list was generated by asking a live
  PostgreSQL rather than written from memory, and doing so removed four names
  that look obviously pure — `date_trunc`, `to_char`, `concat`, and `length`
  (whose two-argument encoding form is stable). Casts are declined for the same
  reason: `timestamptz::date` reads the session TimeZone and cannot be indexed,
  while the identical cast on a plain `timestamp` can, and the plan does not say
  which column type is in play.

  Every suggestion was verified end to end against a live PostgreSQL 18 on a
  600k-row table: build what the advisor proposes, re-run `EXPLAIN`, and confirm
  the planner actually abandons the sequential scan — 1.9× to 1461× faster
  across the cases, with the advice then retiring itself.
